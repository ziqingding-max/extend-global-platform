...
- **管理门户 (Admin Portal)**: `admin.extendglobal.ai` - 供内部运营、财务、销售团队使用。
- **客户门户 (Client Portal)**: `app.extendglobal.ai` - 供客户 HR 和财务联系人使用。
- **员工门户 (Worker Portal)**: `worker.extendglobal.ai` - 供 EOR 员工和独立承包商使用。
...
- **独立入口**：通过 `worker.extendglobal.ai` 访问，与 Admin/Client Portal 隔离。
