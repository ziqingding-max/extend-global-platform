...
基于《B2B2B 系统架构与角色职责重构方案 v3.1》，本计划将整个重构工作拆解为 5 个独立并行的任务组（Task Groups）。每个任务组都具有高度的内聚性，涉及的文件和逻辑相对独立，确保多个 Task 可以并行开发而不产生代码冲突。
...
| 功能点 | 涉及文件 | 详细说明 |
| :--- | :--- | :--- |
| 导航栏重排 | `client/src/components/Layout.tsx` | 将 "Partners" 移至 `useNavGroups` 返回数组的第 1 位，重命名为 "Partner Hub"。将 "Customers" 移至 "Client Directory" 分组。 |
| Invoices Tab 隔离 | `client/src/pages/Invoices.tsx` | 将原有的 `layerFilter` 下拉框改为强制的顶级 Tabs（Layer 1 / Layer 2 / Direct）。确保每个 Tab 仅请求对应 `invoiceLayer` 的数据。 |
| Customers 默认分组 | `client/src/pages/Customers.tsx` | 默认开启 "Group by Partner" 视图，强化 B2B2B 的视觉感知。 |
| Admin CP 表单精简 | `client/src/pages/ChannelPartners.tsx` | 在 Admin 创建/编辑 CP 的表单中，移除品牌白标（Logo、颜色）和银行账户的输入项，仅保留商务信息（域名、底价）。 |
...
| 功能点 | 涉及文件 | 详细说明 |
| :--- | :--- | :--- |
| CP 客户 CRUD | `server/cp-portal/routers/cpPortalClientsRouter.ts`<br>`client/src/pages/cp-portal/CpPortalClients.tsx` | 后端新增 `create`, `update` procedure。前端将只读列表改为完整的管理界面（含创建表单、合同上传）。 |
| 员工资料协助编辑 | `server/cp-portal/routers/cpPortalClientsRouter.ts`<br>`client/src/pages/cp-portal/CpPortalEmployees.tsx` | 后端新增 `updateEmployeeInfo` procedure（仅限非硬数据）。前端增加编辑入口。 |
| 员工档案编辑锁定 | `server/cp-portal/routers/cpPortalClientsRouter.ts`<br>`server/portal/routers/portalEmployeesRouter.ts` | 在后端 update 接口中增加前置校验：`if (employee.status === 'pending_review') throw TRPCError('LOCKED')`。前端根据状态禁用编辑按钮。 |
...
| L2 收款与 Wallet | `server/cp-portal/routers/cpPortalInvoicesRouter.ts`<br>`server/cp-portal/routers/cpPortalWalletRouter.ts` | 后端新增 `markPaid` 和 `payWithWallet` procedure（调用现有的 walletService）。前端增加相应按钮。 |
...
### Task Group E: 底层双层发票引擎改造与 EG-DIRECT 适配
...
| 释放任务双层隔离 | `server/services/depositRefundService.ts`<br>`client/src/pages/admin/ReleaseTasks.tsx` | 确保 Admin 执行的 Release Tasks 只操作 CP 的 Frozen Wallet，而 CP Portal 执行的 Release Tasks 只操作 Client 的 Frozen Wallet。 |
...
1. **第一批次 (Day 1-3)**: 同时启动 Task Group A (Admin UI)、Task Group C (CP 客户赋权) 和 Task Group E (底层引擎改造)。这三个组互不干扰，分别涉及前端展示、CP 路由和底层服务。
